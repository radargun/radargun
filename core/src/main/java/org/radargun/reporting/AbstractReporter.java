...
public abstract class AbstractReporter implements Reporter {
   ...
   @Override
   public boolean run(MainConfig mainConfig, Collection<Report> reports, int returnCode) throws Exception {
      String className = this.getClass().getSimpleName();
      boolean generateReport = skipOnStageFailures ? returnCode == 0 : true;
      if (generateReport) {
         log.info("Running reporter " + className);
         run(mainConfig, reports);
         return true;
      } else {
   ...
   public abstract void run(MainConfig mainConfig, Collection<Report> reports) throws Exception;
}
