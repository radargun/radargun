...
public class PrivateLogLogicTest {
   ...
   public void testMixedOperations() throws Exception {
      ...
      assertEquals(getClassProperty(PrivateLogLogic.class, logic, "txModifications", Collection.class).size(), 7);
      ...
      Map<Long, PrivateLogLogic.OperationTimestampPair> timestamps = getClassProperty(PrivateLogLogic.class, logic, "timestamps", Map.class);
      assertEquals(timestamps.size(), 1);
      assertEquals(timestamps.get(0l).operationId, 6);
      assertEquals(getClassProperty(PrivateLogLogic.class, logic, "txModifications", Collection.class).size(), 0);
   }
   ...
   public void testRemoveOperationsFirst() throws Exception {
      ...
      // No test errors are expected
      assertNull(getFailureManager(logic).getError(true));
      assertEquals(getClassProperty(PrivateLogLogic.class, logic, "txModifications", Collection.class).size(), 3);
