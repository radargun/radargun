...
public class AbstractLogLogicTest {
   ...
   public void testInvokeTxFailures() throws NoSuchFieldException, IllegalAccessException, InterruptedException {
      ...
      logic.keySelectorRandom = mock(Random.class);
      setClassProperty(AbstractLogLogic.class, logic, "txRolledBack", true);
   ...
   public void testWriteStressorLastOperation() throws NoSuchFieldException, IllegalAccessException {
      ...
      AbstractLogLogic logic = createLogic(new GeneralConfiguration(), new LegacyLogicConfiguration(), new LogLogicConfiguration(), cache);
      logic.keySelectorRandom = mock(Random.class);
      setClassProperty(Random.class, logic.keySelectorRandom, "seed", new AtomicLong(123));
   ...
   public void testAfterCommitFailures() throws Exception {
      ...
      doThrow(TestUtils.TestException.class).when(logic).checkedRemoveValue(anyLong(), anyObject());
   ...
   private AbstractLogLogic createLogic(GeneralConfiguration gc, LegacyLogicConfiguration lc, LogLogicConfiguration llc, BasicOperations.Cache cache) throws NoSuchFieldException, IllegalAccessException {
      ...
      setClassProperty(BackgroundOpsManager.class, manager, "basicCache", cache);
      AbstractLogLogic logic = Mockito.mock(AbstractLogLogic.class, CALLS_REAL_METHODS);
      setClassProperty(AbstractLogLogic.class, logic, "nonTxBasicCache", cache);
      setClassProperty(AbstractLogLogic.class, logic, "basicCache", cache);
      setClassProperty(AbstractLogLogic.class, logic, "keySelectorRandom", mock(Random.class));
      setClassProperty(AbstractLogLogic.class, logic, "keyRange", new Range(0, 1));
      setClassProperty(AbstractLogic.class, logic, "manager", manager);
      setClassProperty(AbstractLogic.class, logic, "log", mock(Log.class));
      setClassProperty(AbstractLogic.class, logic, "keyGenerator", new TestUtils.SimpleStringKeyGenerator());
      setClassProperty(AbstractLogic.class, logic, "transactionSize", gc.transactionSize);
   ...
   private FailureManager getFailureManager(AbstractLogic logic) throws NoSuchFieldException, IllegalAccessException {
      BackgroundOpsManager manager = getClassProperty(AbstractLogic.class, logic, "manager", BackgroundOpsManager.class);
