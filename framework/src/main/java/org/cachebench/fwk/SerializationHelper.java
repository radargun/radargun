...
//      byte[0]=(size & 0x000F);
   ...
   public static byte[] prepareForSerialization(Serializable currentStage) throws IOException {
      //todo reuse buffers here
      byte[] bytes = serializeObject(currentStage);
      //TODO! very ugly
      byte[] toSend = new byte[bytes.length + 4];
      byte[] size = intToByteArray(bytes.length);
      //todo use byteBuffer.writeInt etc
