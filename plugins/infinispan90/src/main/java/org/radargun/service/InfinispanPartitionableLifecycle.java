...
public abstract class InfinispanPartitionableLifecycle extends InfinispanKillableLifecycle implements Partitionable {

   private int myWorkerIndex = -1;
   private Set<Integer> initiallyReachable;

   public InfinispanPartitionableLifecycle(InfinispanEmbeddedService wrapper) {
      super(wrapper);
   }
   ...
   protected void setPartitionInChannel(JChannel channel, int workerIndex, Set<Integer> members) {
      ...
      if (partition == null) {
         ...
         try {
            channel.getProtocolStack().insertProtocol(partition, ProtocolStack.Position.ABOVE, TP.class);
         } catch (Exception e) {
